...
\begin{align*}
P(\alpha \geq k) &\leq \binom n k (1-p)^{\binom k 2} \\
&\leq \left ( \frac {en}{k} \cdot (1-p)^{\frac{k-1}2} \right )^k \\
&\leq \left ( \frac {en}k \cdot e^{\frac{-p(k-1)}{2}} \right )^k.
\end{align*}
Si $pk \geq 2 \log (n)$, i.e., $k \geq \frac{2 \log n }{p}$, luego
\begin{align*}
\frac{e \cdot e^{\log (n)}}{k} \cdot e^{\frac{-p (k-1)}{2}} &\leq \frac{e^{1 + \log n - \log n + \frac p 2}}{k} \\
&\leq \frac{e^{1 + p/2}}{k} \\
&\leq e^{\frac 3 2}{k} \leq \frac 5 k.
\end{align*}
Por lo tanto,
\[
...
Equivalentemente,
\[
    P(\alpha < k) \longrightarrow 1, \quad (n \to \infty).
\]
\end{proof}
...
\begin{theorem}
Si $G$ es un grafo con $n$ vértices y $d(G) = d$, entonces
\[
...
Ahora, notemos que
\[
    \mathbb{E}(\abs A - e(G[A])) = pn (1 - \frac {pd}2).
\]
...
\[
    P(\{A | \abs{A} - e(G[A]) \geq p n ( 1 - \frac {pd}{2})\}) > 0,
\]
...
Como bién ilustra la demostración anterior, el \textit{método de alteración} es la técnica de obtener un conjunto $A$ no cumple exactamente la propiedad que buscábamos, pero ``alterándolo'' en $A'$ se obtiene lo que queríamos.

\begin{theorem}[Erdös]
Para todo $k \in \naturals$, existe un grafo $G$ con $n$ vértices tal que
\[
    \chi (G) \geq k \quad \text y \quad g(G) \geq k.
\]
\end{theorem}
\begin{proof}
Sea $k \in \naturals$ fijo. Consideremos $\mathcal G = \mathcal G (n, p_n)$ con $p_n := n^{-1 + \varepsilon}$, donde $\varepsilon := \frac 1 k$. Consideremos la variable aleatoria $X$ en $\mathcal G$ que cuenta el número de ciclos de tamaño a lo más $k-1$. Por definición, existen $n^i$ ciclos de tamaño $i$ contenidos en el grafo completo $K_n$. Luego
\[
    \mathbb{E} (X) \leq \sum_{i = 3}^{k-1} n^i p^i = \sum_{i = 3}^{k-1} n^{\varepsilon i} < k n^{\varepsilon(k-1)}.
\]
...
\]
así, $X < \frac n 2$ con probabilidad alta.
...
Antes, observemos que podemos interpretar el número de Ramsey $R(3, k)$ como el mínimo $n$ tal que todo grafo con $n$ vértices libre de triángulos satisface $\alpha (G) \geq k$. En efecto, si $n = R (3,k)$, podemos considerar el grafo completo $K_n$, e incrustamos $G$ en $K_n$ pintando las aristas de $G$ de color rojo, y las aristas de su complemento $\bar G$ de azul. Si $G$ es libre de triángulos, $K_n$ es libre de $K_3$-rojo, pero entonces tiene un $K_k$-azul, equivalentemente, $\bar G$ contiene un $K_k$, i.e., $G$ tiene con conjunto de $k$-vértices independientes.
...
\end{theorem}
\begin{proof}
Tomemos $n := \left ( \frac{k}{4 \log k} \right )^{\frac 3 2}$. Consideremos $\mathcal G = \mathcal G (n, p)$ con $p = n^{-\frac 2 3}$. Notar que $pk = 4 \log k$. El teorema anterior dice que
\[
    \alpha (G) \leq \frac{2 \log n}{p} < k , \quad G\in \mathcal G
\]
con probabilidad alta. Si $n < k^2$ si y solo si, $\log n < 2 \log k$.

Sea $X$ la variable aleatoria en $\mathcal G$ tal que $X(G)$ es el número de triángulos de $G$. Tenemos que
\[
    \mathbb{E} (X) = \binom n 3 p^3 \leq \frac{n^3}{6}p^3 = \frac n 6.
\]
Luego por la desigualdad de Markov \ref{lema:desigualdad de markov},
\[
    P(X \geq \frac n 2) \leq \frac{2}{n} \mathbb{E} (X) \leq \frac 1 3.
\]
Por lo tanto, existe un grafo $G'$ con $n$ vértices y $\alpha (G') < k$, que contiene a lo más $\frac n 2$ triángulos. Y quitando a lo más $\frac n 2$ triángulos de $G'$, obtenemos un $G''$ con $\alpha (G'') < k$, libre de triángulos y $V(G'') \geq \frac n 2$.
\end{proof}
