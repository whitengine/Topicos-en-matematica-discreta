...
\[
    \forall i \in [n] , P(A_i) \leq x_i \prod_{A_i A_j \in E(\mathcal G)} (1 - x_j) \quad \Longrightarrow \quad p (\bicap_{i \in [n]} A_i^c) \geq \prod_{i \in [n]} (1-x_i) > 0.
\]
\end{lemma}
...
\[
    P (\bicap_{S \cong K_3} A_S \cap \bigcap_{T \cong K_r} B_T) > 0.
\]
...
y
\[
    P(B_T) = (1-p)^{\binom k 2} \leq \exp (- \frac{p k ^2}{4}) \leq \€xp (-4k \log k) < n^{-2k} \leq n^{-k}(1-2 p^3)^{k^2n} (1- n^{-k})^{k^2 n^{k-2}} \leq x_S \prod_{S U \in E(G)} (1-X_U).
\]
