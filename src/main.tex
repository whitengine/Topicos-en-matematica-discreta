...
Demuestre que todo grafo $G$ con $n$ vértices y al menos $n^2/4$ aristas contiene un grafo $H$ con al menos $k$ vértices y $\delta(H)\geq v(H)/2$.
\end{exercise}
\begin{solution}
...
\item En el peor caso, los $\binom k 2$ pares son poco densos. En este caso
\[
...
\]
se tiene que el par $(X,Y)$ es $\frac{2 \varepsilon}{\alpha}$-regular. Además, por $\varepsilon$-regularidad del par $(A,B)$, se tiene que
\[
    d(X,Y) \geq d (A,B) - \varepsilon.
\]

\end{lemma}
\begin{proof}
La última afirmación es clara. Veamos la primera. Sean $Z \subset X$ y $W \subset Y$ tales que $\abs Z \geq \frac{2\varepsilon}{\alpha} \abs X$ y $\abs W \geq \frac{2\varepsilon}{\alpha} \abs Y$, entonces $\abs Z \geq 2 \varepsilon \abs A$ y $\abs W \geq 2 \varepsilon \abs B$. Luego por $2\varepsilon$-regularidad del par (A,B), se tiene que
\[
    \abs{d(Z,W) - d(A,B)} \leq 2 \varepsilon.
\]
Además, por $\alpha$-regularidad del par $(A,B)$, se tiene que
\[
    \abs{d(X,Y) - d(A,B)} \leq \alpha.
\]
Juntando ambas desigualdades tenemos que:
\begin{align*}
\abs{d(Z,W) - d(X,Y)} &\leq \abs{ d(Z, W) - d(A,B)} + \abs{d(X,Y) - d(A,B)} \\
    &\leq 2 \varepsilon + \alpha \leq 2 \frac{\varepsilon}{ \alpha}
\end{align*}
\end{proof}

\begin{remark}
Creo que no vale que $(X,Y)$ sea $2\frac \varepsilon \alpha$-regular, si no que es $\varepsilon '$-regular con $\varepsilon ' := \max \{ \frac{\varepsilon}{\alpha}, 2 \alpha \}$, pues esta es la constante a la cual se le puede probar regularidad de manera sencilla. Ver como modificar los siguientes resultados que usan el lema de slicing.
\end{remark}
...
\begin{proof}
Sea $\varepsilon > 0$ lo suficientemente pequeño (que depende de $H$ y $\delta$). Aplicamos el Lema de Regularidad de Szémeredi \ref{theorem:lema de regularidad de szemeredi} para $\varepsilon$ y $m \geq \frac 1 \varepsilon$; obtenemos la partición $\varepsilon$-regular $V(G) = V_0 \coprod V_1 \coprod \cdots \coprod V_k$ con $m \leq k \leq M$ para todo grafo con $\abs G \geq M$.

Luego consideramos el ``grafo reducido'' $R$ con parámetros $\varepsilon$ y $\delta$, y vértices $w_1, \ldots, w_k$. Sea $r = \chi (H) - 1$. Si $K_{r+1} \subset R$, entonces $H \subset G$ por el Lema de Inmersión general aplicable \ref{lema:regularidad de szemeredi - lema inmersion aplicable}, lo cual nos lleva a una contradicción. Es decir, $R$ es $K_{r+1}$-libre.
...
\end{align*}
contradicción. Con lo cual, podemos asumir que $R$ está $t$-cerca de ser $r$-partito. Es decir, hay una $r$-partición
\[
...
\[
    \underbrace{t \cdot \left ( \frac n k \right )^2}_{\leq 5 \delta n^2} + \underbrace{(\delta + 3 \varepsilon)n^2}_{< 5 \delta n^2}
\]
...
Para todo $\varepsilon > 0$, existe $n_0 \in \naturals$ tall que si $n \geq n_0$ y $A \subset \{1 , \ldots, n\}$ con $\abs A > \varepsilon n$, entonces $A$ contiene una \textit{$3$-progresión aritmética}\footnote{En general, una \textbf{$k$-progresión aritmética} es una secuencia de enteros $a, a+d, a + 2d , \ldots, a + (k-1)d$.}.
\end{theorem}

\begin{lemma}[Lema de remoción de triángulos]\ref{lema:lema de remocion de triangulos}
...
\[
    V(G) = V_0 \coprod V_1 \coprod \cdots \coprod V_k.
\]
Consideremos el grafo reducido $R$ con parámetros $\varepsilon$ y $\delta$.

Supongamos que $R$ tiene al menos un triángulo $K_3$. Aplicando el Lema de conteo general \ref{lema:lema de conteo general - corolario del lema de regularidad de szemeredi} para $H = K_3$, tenemos que $G$ tiene al menos:
\[
    \delta^3 \cdot \left ( \frac{(1 - \varepsilon)n}{k} \right )^3 > \frac{\delta^3}{2} \frac{(1- \varepsilon)^3}{M^3} \cdot n^3 > \beta n^3
\]
triángulos, donde $\beta < \frac{\delta^3}{2} \frac{(1- \varepsilon)^3}{M^3}$.

Con lo cual, si $G$ tiene a lo más $\beta n^3$ triángulos, el párrafo anterior nos dice que $R$ no tiene triángulos. Con lo cual, al remover $\leq \underbrace{(\delta + \varepsilon)}_{< \alpha}n^2$ aristas de $G$ nos quedamos sin triángulos.

En general si $\abs G < m$, podemos achicar a $\beta$ para que valga el enunciado en general.
\end{proof}
