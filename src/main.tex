...
\begin{definition}
El \textbf{grafo de Turán} $T_k (n)$ es el grafo $k$-partito completo con la mayor cantidad de aristas. Notamos
\[
    t_k(n) := e(T_k (n)).
\]
\end{definition}
...
\begin{obs}
Si $n$ es un múltiplo de $k$,
\[
    t_k (n) = \frac{n^2}{k^2} \cdot \binom k 2 = (1- \frac 1 k)\cdot \frac{n^2}{2}.
\]
\end{obs}


\begin{theorem}[Turán, 1941]
Sean $n,k \in \naturals$, $G$ un grafo $K_{k}$-libre con $n$ vértice. Entonces
\[
    e(G) \leq t_k (n).
\]
...
\begin{proof}
Hagamos inducción en $n$. Para $n \leq k-1$ es trivial. Sea ahora $G$ con $n \geq k$ que a su vez es $K_{k}$-libre y arista maximal. Esto implica que agregar cualquier arista hace aparecer un $K_{k}$ como subgrafo. Entonces $G$ contiene un $K_{k-1}$. Sea $A$ el conjunto de vértices de un subgrafo $K_{k-1}$ en $G$. Consideremos luego $G' = G\setminus A$. El grafo $G'$ es $K_{k}$-libre y tiene $n-k+1$ vértices. Cada $x \in V(G')$ tiene a lo más $k-2$ vecinos en $A$ dentro del grafo $G$, pues $G$ es $K_{k}$-libre. Luego por hipótesis inductiva:
\[
    e(G') \leq t_k(n-k+1).
\]
Si juntamos esto con la hipotesis inductiva, tenemos que
\[
    e(G) \leq e(G') + \overbrace{(n-k+1) \cdot (k-2)}^{\text{aristas entre $A$ y $V(G')$}} + \binom {k-1} 2    \leq t_k (n-k+1) + (n-k+1) \cdot (k-2) + \binom {k-1} 2 = t_k (n).
\]

TAREA: ESCRIBIR LA SEGUNDA PARTE DE LA DEMOSTRACIÓN (EL ADEMAS...)
\end{proof}
