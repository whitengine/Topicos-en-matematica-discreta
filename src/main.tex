...
\]
como la familia de grafos $G$ tales que existe una equipartición $V(G)  = A_1 \coprod \cdots \coprod A_l$ con $\abs{A_i}$ todos del mismo tamaño $n$, y un etiquetamiento de los vértices $V(H) = \{w_1, \ldots, w_l\}$ tal que para cada $w_i w_j \in E(G)$, el par $(A_i, A_j)$ es un par $\varepsilon$-regular y además $d(A_i, A_j)\geq\delta$.
...
Sea $H$ un grafo y $\delta >0$. Defina $r = \chi (H)$. Entonces, existen $\varepsilon > 0$ y $n \in \naturals$ tales que
\[
    G \in \mathcal G (K_r , n , \varepsilon, \delta) \quad \Rightarrow \quad H \subset G.
\]
\end{lemma}
\begin{proof}
Como $H \subset K_r (t)$ con $t = \abs H$, el Lema \ref{lema:regularidad de szemeredi - lema inmersion aplicable} garantiza que para todo $\delta ' > 0$ existen $\varepsilon ', n'_0$ tales que
\[
    G \in \mathcal G (K_r (t), n' , \varepsilon ' , \delta '), n' \geq n'_0 \quad \Rightarrow \quad H \subset K_r (t) \subset G.
\]
Concluimos gracias al siguiente ejercicio:
\begin{exercise}
Demostrar que existen $n \in \naturals$, $\varepsilon$ y $\delta'$ tales que
\[
    \mathcal G (K_r, n , \varepsilon, \delta) \subset \mathcal G (K_r (t), n', \varepsilon ', \delta ').
\]
\end{exercise}
\begin{solution}
Tomemos $n = n' t$, para $n'\geq n_0'$ lo suficientemente grande. Fijemos un etiquetamiento $K_r = \{w_1, \ldots, w_r\}$ tal que $K_r (t) = \{w_i^j\}_{1 \leq i \leq r}^{1 \leq j \leq t}$ con $w_i^j w_{i'}^{j'} \in E(K_r (t))$ si y solo si $w_i w_{i'} \in E(K_r)$. Entonces si $G \in \mathcal G (K_r , n, \varepsilon, \delta)$, con equipartición $V(G) = \coprod_{i = 1}^r V_i$. Se sigue que podemos subdividir la partición: cada $V_i = \coprod_{j = 1}^t V_i^t$ en otra equipartición con partes de cardinal $n'$.
